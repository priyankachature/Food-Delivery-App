      ...
      <div className="max-w-4xl mx-auto space-y-6 sm:space-y-8">
        ...
        <div className="text-center pb-6 sm:pb-8 border-b border-slate-200/80">
          ...
          <h1 className="text-2xl sm:text-4xl font-black text-slate-900 tracking-tight leading-tight">
            Arriving in <span className="text-orange-600">30 - 45 Mins</span>
          </h1>
          <p className="text-sm text-slate-500 mt-2.5 font-medium flex flex-wrap items-center justify-center gap-1">
            Order Id : <span className="font-mono text-slate-800 font-bold bg-slate-100 border border-slate-200/60 px-2 py-0.5 rounded">#{orderId}</span>
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-start">
          ...
          <div className="lg:col-span-7 space-y-4 sm:space-y-6">
            ...
            {/* Status Card */}
            <div className="bg-white rounded-2xl p-5 sm:p-6 border border-slate-100 shadow-sm transition-all hover:shadow-md/5 flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-sm sm:text-base font-bold text-orange-700 mt-0.5 tracking-wide px-2.5 py-0.5 rounded bg-orange-50 inline-block max-w-full truncate">
                  {order?.status}
                </p>
              </div>
            </div>
              ...
              <div className="flex-1 min-w-0">
                ...
                {order?.address ? (
                  <div className="text-sm text-slate-600 space-y-1.5 leading-relaxed wrap-break-word font-medium">
                    ...
                    <div className="inline-flex items-center gap-1.5 bg-slate-50 border border-slate-100 px-2.5 py-1 rounded-lg text-xs font-bold text-slate-700 mt-1">
                      <span>📞</span> <span>{order.address.mobile}</span>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-5 w-full space-y-4 sm:space-y-6">
              ...
              <div className="space-y-3 max-h-44 overflow-y-auto pr-1">
                {order?.items?.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-start text-sm gap-4">
                    <span className="text-slate-700 font-medium wrap-break-word max-w-[70%]">
                      {item.menuItem?.name || item.name} <span className="text-slate-400 font-bold text-xs whitespace-nowrap">x{item.quantity}</span>
                    </span>
            ...
            <div className="w-full overflow-hidden">
              <OrderSummary order={order} showActionButton={false} />
            </div>
          </div>
        ...
        <div className="text-center space-y-4 max-w-md mx-auto pt-4 px-2">
          <button
            ...
            className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold px-6 py-3.5 rounded-xl shadow-md transition-all transform active:scale-[0.99] tracking-wide text-sm"
          >
